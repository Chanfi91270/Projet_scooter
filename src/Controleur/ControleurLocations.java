...
public class ControleurLocations {
    ...
    public boolean louerScooter(JFrame parent) {
        try {
            ...
            Date dateDebut;
            while (true) {
                String strDateDebut = JOptionPane.showInputDialog(parent, "Entrez la date de début (format : jj/mm/aaaa) :");
                if (strDateDebut == null) return false; // annulation
                try {
                    dateDebut = new java.text.SimpleDateFormat("dd/MM/yyyy").parse(strDateDebut);
                    break;
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(parent, "Format invalide. Veuillez utiliser le format jj/mm/aaaa.", "Erreur", JOptionPane.ERROR_MESSAGE);
                }
            }
