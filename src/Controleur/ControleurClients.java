    ...
    public void modifierClient(JFrame parent) {
        try {
            ...
            if (client != null) {
                ...
                String nouveauTel = null;
                boolean valide = false;
                while (!valide) {
                    nouveauTel = JOptionPane.showInputDialog(parent, "Nouveau téléphone (doit être un entier) :", client.getNumTel());
                    if (nouveauTel == null) {
                        // Annulation par l'utilisateur : on sort sans changer le téléphone
                        break;
                    }
                    try {
                        // Essayer de convertir en int pour valider
                        Integer.parseInt(nouveauTel);
                        valide = true;
                    } catch (NumberFormatException e) {
                        JOptionPane.showMessageDialog(parent, "Le numéro de téléphone doit être un entier. Veuillez réessayer.");
                    }
                }

                if (nouveauNom != null) client.setNom(nouveauNom);
                if (nouveauPrenom != null) client.setPrenom(nouveauPrenom);
                if (valide && nouveauTel != null) client.setNumTel(nouveauTel);
